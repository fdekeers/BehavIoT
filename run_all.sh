...
# 5.1. Train
python3 $PIPELINE_DIR/s5_periodic_filter.py -i $DATA_DIR/idle-2021-train-std/ -o $MODEL_DIR/filter_apr20 > $LOGS_DIR/5-periodic-event-inference/1-train-idle.log 2> $LOGS_DIR/5-periodic-event-inference/1-train-idle.error
...
python3 $PIPELINE_DIR/s6_binary_model_whostname.py -i $DATA_DIR/train-filtered-std/ -o $MODEL_DIR/binary > $LOGS_DIR/6-user-event-inference/2-train-binary-whostname.log 2> $LOGS_DIR/6-user-event-inference/2-train-binary-whostname.error
python3 $PIPELINE_DIR/s6_binary_predict_whostname.py -i $DATA_DIR/routines-filtered-std/ -o $MODEL_DIR/binary > $LOGS_DIR/6-user-event-inference/3-predict-routines-binary-whostname.log 2> $LOGS_DIR/6-user-event-inference/3-predict-routines-binary-whostname.error
# Without hostname
